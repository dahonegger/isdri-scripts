...
while ~feof(fid)   
    ...
    if ~isempty(double(cell2mat(tmp(6))))
        Hs(k) = double(cell2mat(tmp(6)));
    else
        Hs(k) = nan;
    end
    if ~isempty(double(cell2mat(tmp(14))))
        TpAve(k) = double(cell2mat(tmp(14)));
    else
        TpAve(k) = nan;
    end
    if ~isempty(double(cell2mat(tmp(8))))
        TpSwell(k) = double(cell2mat(tmp(8)));
    else
        TpSwell(k) = nan;
    end
