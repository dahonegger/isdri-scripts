function cube2png_guadalupe_rips_large_domain(cubeFile,pngFile)
...
axisLimits              = [-2000 -500 -1500 1500]; % in meters
...
[AZI,RG] = meshgrid(Azi,Rg(16:upperCutoff));

% interpolate onto a smaller cartesian grid
xC = axisLimits(3):axisLimits(4);
yC = axisLimits(1):axisLimits(2);
...
if size(timeInt,2) == 64
    ...
    tC = interp2(AZI,RG,double(timex(16:upperCutoff,:)),aziC',rgC');
    timexCell{1} = tC;
...
elseif size(timeInt,2) > 64*2
    ...
    for i = 1:64:(floor(size(data,3)/64))*64 - 64
        tC = interp2(AZI,RG,double(mean(data(16:upperCutoff,:,i:i+64),3)),aziC',rgC');
        timexCell{ii} = tC;
...
for IMAGEINDEX = 1:numel(timexCell)
    ...
    fig.PaperPosition = [0 0 10 7.2];
    fig.Units = 'pixels';
    fig.Position = [0 0 1280 720];
%     axRad = axes('position',[-0.1081    0.1167    0.7750    0.8150]);
    axRad = axes('position',[-0.11    0.1167    0.7750    0.8150]);
    axTide = axes('position',[0.51    0.7269    0.4200    0.2053],'fontsize',8);
    axWind = axes('position',[0.64    0.4000    0.1544    0.2547],'fontsize',8);
    axWaves = axes('position',[0.515    0.1358    0.4169    0.2011],'fontsize',8);
...
%     plot(xJMC,yJMC,'b.','MarkerSize',20)
    ...
    plot([nowTime nowTime],[0 6],'-','color',[.5 .5 .5],'linewidth',2);
    xlim([nowTime-4 nowTime+4])
    ylim([0 6])
