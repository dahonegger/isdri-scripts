...
minRangeToAnalyze = 2000;           % Min range to consider (in pixels)
maxAziToAnalyze = 360;           % Max azimuth to consider (in degrees)
dRange = 500;                       % Range decimation of analysis (in pixels)
...
minAziForDiag = 140;
maxAziForDiag = min(330,numel(Cube.Azi)*aziResolution);
trigSkipDiagnostic = sum(squeeze(mean(diff(Cube.data(:,(minAziForDiag:maxAziForDiag)/aziResolution,:),[],2))) < 1);
