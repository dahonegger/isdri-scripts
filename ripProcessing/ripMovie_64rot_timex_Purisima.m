...
startTime = '20170907_0430';
endTime = '20170912_0000';

Hub = 'E';
baseDir = [Hub ':\purisima\processed\'];
saveFolder = ['E:\purisima\postprocessed\ripVideos\' startTime '-' endTime];
mkdir(saveFolder)

%% define figure parameters
colorAxisLimits = [95 210];
% colorAxisLimits = [75 170];
% XYLimits = [-1300 -500; -300 2900];
% XYLimits = [-1500 -500; -1000 1000];
% XYLimits = [-2500 -500; -3000 3000];%
XYLimits = [-5800 3100; -5800 5800];
...
for iCube = 1:length(cubeListAll);
    dd = datevec(doy2date(str2num(cubeListAll(iCube).name(14:16)),2017));
    dnList(iCube) = datenum([dd(1), dd(2), dd(3),...
        str2num(cubeListAll(iCube).name(17:18)),...
        str2num(cubeListAll(iCube).name(19:20)),0]);
end
...
for iCube = 1:length(cubeList)
    ...
    [~,xCutoff] = min(abs(Rg - 6000));
    Azi1 = Azi-heading;
    AziIndices = find(Azi1 > -33 & Azi1 <200);
    [AZI,RG] = meshgrid(Azi(AziIndices),Rg(xMin:xCutoff));
    ...
    if rotation == 0
        ...
        if size(timeInt,2) == 64
            clear timex
            load(cubeName,'data')
            timex = double(mean(data(1:xCutoff,AziIndices,:),3));
            timexCell{1} = timex;
        ...
        elseif size(timeInt,2) > 64*2
            ...
            for iRot = 1:64:(floor(size(data,3)/64))*64 - 64
                ...
                timexCell{ii} = double(mean(data(1:xCutoff,AziIndices,iRot:iRot+64),3));
