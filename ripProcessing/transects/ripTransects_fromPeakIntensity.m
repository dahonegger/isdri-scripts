% ripTransect_fromPeakIntensity.m
% 1/5/2018
% This code finds the peak intensity, smooths it using a moving average,
...
for iDay = 2 + numel(dayFolder)-numDays:numel(dayFolder) %loop through days
            ...
            if sum(iRun ~= skip) == length(skip)
                ...
                if exist('Rg')
                    ...
                    for tt = 1:size(timex448,3)
                        ...
                        idxSmoothedMI = movmean(idxMaxIntensity,800);
                        ...
                        for iY = 1:length(yC)
                            TT(iY) = tC(idxSmoothedMaxIntensity(iY),iY);
                            T30(iY) = tC(idxSmoothedMaxIntensity(iY)-30,iY);
    ...
    save(fullfile(saveDir,output_fname),'txIMat','txIMat_30','txIMat_75','txIMat_100','txIMat_150','txIMat_200','txDn','yC','idxMaxI','-v7.3')
    ...
    txIMat = []; txIMat_30 = []; txIMat_75 = []; txIMat_100 = []; txIMat_150 = []; txIMat_200 = []; txDn = []; idxMaxI = [];
    %
end
